...
gcloud run deploy $SERVICE_NAME \
  ...
  --project $PROJECT_ID \
  --region $REGION \
  --set-env-vars="SLACK_SIGNING_SECRET=$SLACK_SIGNING_SECRET,SLACK_BOT_TOKEN=$SLACK_BOT_TOKEN,TARGET_CHANNEL_ID=$TARGET_CHANNEL_ID,GEMINI_API_KEY=$GEMINI_API_KEY,JIRA_URL=$JIRA_URL,JIRA_EMAIL=$JIRA_EMAIL,JIRA_API_TOKEN=$JIRA_API_TOKEN,JIRA_BOARD_ID=$JIRA_BOARD_ID,MONGO_DB_NAME=$MONGO_DB_NAME,GOOGLE_CLOUD_PROJECT=$PROJECT_ID,IS_LANGGRAPH_DEV=0" \
